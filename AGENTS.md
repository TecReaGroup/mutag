...
3. 代码中间产物，或者临时文件放到 ./temp 文件夹下
4. 必要时更新 Makefile 文件，必要时更新 Makefile 文件，最主要查看 makefile 里 install 和 run 的实现
5. 给出必要的 log，格式为：[YYYY-MM-DD HH:mm:ss +08:00] [级别] [模块] - 具体信息，持久化保存到 ./log/ 目录下，按日期分文件保存，文件命名格式：log_YYYY-MM-DD.log
6. 所有目录和包名注意符合优秀的命名规范
7. 注释规范：
...
8. 问题尽量使用优雅的方案解决，而不是反复尝试，有时候可以搜索业界优秀的sota是怎么解决的，参考别的解决方案
9. 对于常量，使用大写字母加下划线的方式命名，并且尽量放到文件开头部分，尤其是后续可能需要用户调整的参数
10. 对于对话给出的文件路径需要给出 [file_name](file_path:column_number) 的链接，前后需要有空格，方便直接跳转到对应的文件位置
11. 注意 .gitignore 的更新和 .gitkeep 的使用，尤其是 data 目录下的文件
12. 注意中英双语的支持和相关规范
