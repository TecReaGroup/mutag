  ...
  return <section aria-label={t("Audio tags", "音频标签")} className="relative flex min-w-0 flex-1 flex-col overflow-hidden bg-surface">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      {scanning ? <div className="space-y-3 p-4">{Array.from({ length: 8 }, (_, index) => <div key={index} className="flex gap-4"><SkeletonLine className="h-12 flex-1" /><SkeletonLine className="h-12 flex-1" /></div>)}</div> : file ? <div className="space-y-3 p-4">
        {file.pendingArtwork && <div className="rounded border border-border p-3"><p className="text-xs">{t("Generated artwork · pending acceptance", "生成图片 · 等待审核")}</p><img src={file.pendingArtwork.image} alt={t("Generated artwork preview", "生成图片预览")} className="my-2 h-40 w-40 object-contain" /><p className="text-xs text-muted-foreground">{file.pendingArtwork.filenames.join(" / ")} · {t("Stored in .temp until accepted. Existing files are preserved.", "暂存于 .temp，Accept 后写入，已有文件保留。")}</p></div>}
