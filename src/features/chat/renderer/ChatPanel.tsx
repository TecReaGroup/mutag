  ...
  return <>
    ...
    <div className="shrink-0 space-y-1.5 border-t border-border p-2">
      ...
      <div className="flex items-center gap-2">{disabled ? <p className="min-w-0 flex-1 text-[10px] text-warning">{activeCommand ? t("Running command…", activeCommand.progressMessage) : t("Saving changes…", "正在保存修改…")}</p> : dirtyCount > 0 && <p className="min-w-0 flex-1 text-[10px] text-warning">{t("Save or discard pending changes before running commands. You can still chat.", "运行命令前请保存或放弃待确认修改，仍可继续对话。")}</p>}
        {sending ? <Button className="ui-danger-button ml-auto px-2" onClick={onStop} disabled={disabled}>{t("Stop", "停止")}</Button> : <Button className="ml-auto px-2" onClick={onClear} disabled={disabled || !messages.length}>{t("Clear", "清空")}</Button>}
      </div>
      <div className="flex flex-wrap gap-1">{commands.map((command) => <Button key={command.name} className="px-2 py-1 text-[10px] text-primary" disabled={dirtyCount > 0 || inputDisabled} title={t(command.name === "/meta" ? "Complete missing metadata" : command.name === "/image" ? "Download missing artwork" : "Organise audio files", command.description)} onClick={() => { onInput(command.name); inputRef.current?.focus(); }}>{command.name}</Button>)}</div>
