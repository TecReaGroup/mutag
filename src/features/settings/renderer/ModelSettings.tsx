  ...
  return <div className="max-w-2xl space-y-4">
    ...
    {profiles.map((profile) => <Disclosure key={profile.model} title={`${profile.model || t("New model", "新模型")}${profile.model && profile.model === activeModel.model ? t(" · Active", " · 当前模型") : ""}`} expanded={editing === profile.model}
      ...
      {/gemini/i.test(draft.model) && <div className="grid grid-cols-2 gap-3">
        <label className="flex items-center gap-2 text-xs"><input type="checkbox" role="switch" checked={draft.uploadAudio} onChange={(event) => setDraft({ ...draft, uploadAudio: event.target.checked })} />{t("Upload audio", "音频上传")}</label>
        <label className="flex items-center gap-2 text-xs"><input type="checkbox" role="switch" checked={draft.webSearch} onChange={(event) => setDraft({ ...draft, webSearch: event.target.checked })} />{t("Web search", "联网搜索")}</label>
      </div>}
